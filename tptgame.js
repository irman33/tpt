...
function playerBuzzedIn(data) {
  if (!(typeof Games[data.gameCode] == "undefined")) {
    if (Games[data.gameCode].buzzOn) {
      ...
      io.sockets
        .in(data.gameCode)
        .emit("buzzerOff", { playerName: data.playerName, team: data.team });
    }
  }
}
...
function resetBuzzer(data) {
  if (!(typeof Games[data.gameCode] == "undefined")) {
    ...
    io.sockets
      .in(data.gameCode)
      .emit("buzzerOn", { playerName: data.playerName, team: data.team });
  } else {
...
function updateScore(data) {
  ...
  io.sockets
    .in(data.gameCode)
    .emit("scoreUpdate", {
      redScore: Games[data.gameCode].redScore,
      blueScore: Games[data.gameCode].blueScore
    });
}
...
function updateRoster(data) {
  if (!(typeof Games[data.gameCode] == "undefined")) {
    io.sockets
      .in(data.gameCode)
      .emit("rosterUpdate", {
        redTeam: Games[data.gameCode].redTeam,
        blueTeam: Games[data.gameCode].blueTeam
      });
  }
}
...
function disconnect() {
  ...
  if (
    !(allClients[i].playerName == null) ||
    !(allClients[i].playerName == "")
  ) {
